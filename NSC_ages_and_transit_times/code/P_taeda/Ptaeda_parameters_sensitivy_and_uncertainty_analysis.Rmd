...
This Markdown present the code used for calculating the parameters related to the model presented in the Figure 2 of the manucript "Non-structural carbon ages and transit times provide insights in carbon allocation dynamics of mature trees" for the species P. taeda. The parameters are calculated implementing the model "Allometrically Constrained Growth and
Carbon Allocation" ACGCA (Ogle and Pacala 2009). Here it is also presented the code for running the analysis of the uncertainties related to the model parameters in the model output (mean ages and mean transit times) and the sensitivity of the mean ages and transit time to numerical changes in those parametes. 
...
```{r}
setwd("/Users/_dherrera/NSC_ages_and_transit_times/code/P_taeda/")
source("/Users/_dherrera/NSC_ages_and_transit_times/code/script_functions_model_ACGCA.R")

```

## P. taeda parameter calculation 

Paramogle is the dataset of the parameters provided for Ogle and Pacala (2009) for running the model ACGCA and estimating the parameter for the model presented in the figure 2. the vector was contructed with the mean values for P. taeda (first row) and A. rubrum (second row). From here on we will just use the parameters corresponding to P. taeda. 
...
Then we ran the ACGCA model for 800 years (niter) to assure that we selected the parameters at the steady state of the system
```{r}
...
## With this values we proceeded to calculate the transfer coeficients for the model used in the manuscript and described in Figure 2
...
SsL=PFlT # SsL is the fast cycling pool of sugars in the leaves, and 
...
senlT=paraPtadea["Sl"]*flux300yr$Bl*paraPtadea["Cgl"] ## g of gluc per year lost in the litter fall 
dlSl=paraPtadea["Sl"]*flux300yr$Bl*paraPtadea["dl"]; dlSl ##g gluc per year translocated to the E pool from the storage
...
Ssr=PFrT ## g gluc in the fast cycling pool of sugar in the roots
Gr=PFrT*(paraPtadea["Cgr"]/(paraPtadea["Cgr"]+paraPtadea["dr"])); Gr## grams of wood constructed from the allocated carbon to roots
...
PFoT=flux300yr$Fo*flux300yr$E; PFoT## amount of g glucose allocated to other woody compartments per year 
...
senoTgdw=PFoT/(paraPtadea["Crw"]+flux300yr$ds)#g of wood per year lost to close steady state
senoT=PFoT/(paraPtadea["Crw"]+flux300yr$ds)*paraPtadea["Crw"]## g of glucose lost to close steady state
stro=PFoT*(flux300yr$ds/(paraPtadea["Crw"]+flux300yr$ds))## g of glucose per year allocated to storage inthe wood
So=(PFoT/(paraPtadea["Crw"]+flux300yr$ds))/Bo## g of wood per year lost in litter fal from branches and coarse roots 
dsSo=flux300yr$ds*So*flux300yr$Bos### g of gluc per year retranslocated to E from branches and coarse roots 
dwSo=strt+stro-dsSo## g of gluc per year lost in litterfall 
...
ratesPtadea=as.data.frame(t(ratesPtadea)) ### this are the parameters o run the model in Fig 2 and obtain the results for P. taeda shown in the manuscript. This parameter were used in the phyton script (P.taeda_ages_and_transit_times.ipynb) for the computation of ages and transit times. 
```
...
```{r}
source("/Users/_dherrera/NSC_ages_and_transit_times/code/script_function_senes_uncert_ACGCA.R")
```
...
We defined the range of variability of the parameters for the ACGCA model for P. taeda. This ranged were extracted from the supplementary inforamtion provided by Ogle and Pacala 2009. 
...
We prepared the parameters to feer the function modCRL for the Monte Carlo Simulations (MCS) and run the model to propagate the uncertainty from the ACGCA model parameters provided by Klein and Hoch 2009 to the compartmental model presented in figure 2 of te manuscript. 
```{r}
pars1_l=list()
...
```{r}
## we prepare again the list of model parameters to feed the modCRK function 
