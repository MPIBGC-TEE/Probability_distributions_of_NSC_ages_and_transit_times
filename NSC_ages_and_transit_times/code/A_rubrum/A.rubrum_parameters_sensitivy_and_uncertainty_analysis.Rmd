...
```{r}
setwd("/Users/_dherrera/NSC_ages_and_transit_times/code/A_rubrum/")
source("/Users/_dherrera/NSC_ages_and_transit_times/code/script_functions_model_ACGCA.R")

```

## A. rubrum parameter calculation 
...
Then we ran the ACGCA model for 800 years (niter) to assure that we selected the parameters corresponding to a steady state condition for A. rubrum. 
...
ratesPtadea=as.data.frame(t(ratesPtadea)) ### this are the parameters o run the model in Fig 2 and obtain the results for A. rubrum shown in the manuscript. This parameter were used in the phyton script (A.rubrum_ages_and_transit_times.ipynb) for the computation of ages and transit times. 
```
